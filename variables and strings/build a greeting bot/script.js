console.log("Hi there!");
console.log("I am excited to talk to you.");
let bot = "teacherBot";
let botLocation = "the universe";
console.log("Allow me to introduce myself.");

let botIntroduction;
botIntroduction = "My name is " + bot + ".";
console.log(botIntroduction);

let botLocationSentence; /* membuat variable baru */
botLocationSentence = "I live in " + botLocation + ".";/* menambahkan variable sebelumnya yang bernama botLocation */
console.log(botLocationSentence);

/* mengubah isi variable sebelumnya */
bot = "professorBot";

let nicknameIntroduction;
nicknameIntroduction = "My nickname is " + bot + "."
console.log(nicknameIntroduction);

bot = "awesomeTeacherBot";

let newNicknameGreeting;
newNicknameGreeting = "I love my nickname but I wish people would call me " + bot + ".";
console.log(newNicknameGreeting);

let favoriteSubject = "Computer Science";

let favoriteSubjectSentence;
favoriteSubjectSentence = "My favorite subject is " + favoriteSubject + ".";
console.log(favoriteSubjectSentence);

// Log the final farewell message to the console
console.log("Well, it was nice to talk to you. Have a nice day!");